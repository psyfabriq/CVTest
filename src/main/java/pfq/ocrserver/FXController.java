...
public class FXController implements Initializable {
       ...
       double  kernel_b = 7;
	    ...
	    void handleFindContursButtonAction(ActionEvent event) {
	    	Utill.detectObject(bufferedImage, bufferedTemplateImage,20);
	    	
	    	
	    	if(MemoryUtil.getObj("outputImage").isPresent())
	    	test1Frame.setImage(Utill.getImage((Mat)MemoryUtil.getObj("outputImage").get()));
	    	if(MemoryUtil.getObj("matchoutput").isPresent())
		    test2Frame.setImage(Utill.getImage((Mat)MemoryUtil.getObj("matchoutput").get()));
	    	if(MemoryUtil.getObj("img").isPresent())
		    test3Frame.setImage(Utill.getImage((Mat)MemoryUtil.getObj("img").get()));
	    	if(MemoryUtil.getObj("crop").isPresent())
		    test4Frame.setImage(Utill.getImage((Mat)MemoryUtil.getObj("crop").get()));
	    }
