...
public class Utill {
	...
	public static void detectObject(BufferedImage source, BufferedImage template,int number_of_occurrences) {
		 ...
		 try {
			...
			Mat sceneImage        = Utill.BufferedImage2Mat(sources);  
	        ...
	        if (goodMatchesList.size() >= number_of_occurrences) {
	         ...
	         //   Mat cropMat = new Mat(img.width(), resultHeight, CvType.CV_8UC4);
	          ...
	          //  System.out.println(ocvPOut4);
	            ...
	            /*
	            int startX = (int)ocvPOut4.x;
	            int startY = (int)ocvPOut4.y;
	            
	            int width  = (int)ocvPOut2.x-startX;
	            int height = (int)ocvPOut2.y-startY;
	            */
	            Rect rectCrop = getRectangleFromFourCounts(ocvPOut1,ocvPOut2,ocvPOut3,ocvPOut4);
	            System.out.println(rectCrop);
	             
	            Mat croppedImage = new Mat(img.clone(), rectCrop);
	            
	            

	            Imgproc.line(img, ocvPOut4, ocvPOut1, new Scalar(0, 255, 0), 4); // top line
	            Imgproc.line(img, ocvPOut1, ocvPOut2, new Scalar(0, 255, 0), 4);
	            ...
	            MemoryUtil.putObj("outputImage", outputImage);
	            MemoryUtil.putObj("matchoutput", matchoutput);
	            MemoryUtil.putObj("img", img);
	            MemoryUtil.putObj("crop", croppedImage);
	            MemoryUtil.putObj("found", true);
